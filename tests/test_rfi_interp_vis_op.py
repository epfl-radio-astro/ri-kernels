...
def reference(amp, phase, delay, w_freq, start_freq, w_time, start_time, dnu, dt, freqs, a1, a2):
    ...
    weight = jnp.full((len(a1), n_int_t), 1.0 / (len(dnu) * n_int_t), dtype=amp.dtype)  # (n_bl, n_int_t)
    cells = []
...
def test_the_tile_pair_list_holds_each_pair_once():
    ...
    lst = tile_pair_list(n_ant, a1, a2)
    n_tiles = 3
    ...
    for tp, row in enumerate(lst):
        ...
        J = I + rem
        for e in entries:
            i, j = I * TILE + e // TILE, J * TILE + e % TILE
            assert i < n_ant and j < n_ant
            if I == J:
                assert i <= j
            seen[(i, j)] = seen.get((i, j), 0) + 1
    assert all(n == 1 for n in seen.values()), "each pair once"
