...
#define HWY_TARGET_INCLUDE "rfi_delay_kernel.cpp" // this file
...
template <typename T>
HWY_ATTR void
rfi_delay_kernel_opt_tmpl(T scale, std::int64_t i_bl_start,
  ...
  for (std::int64_t i_bl = i_bl_start; i_bl < i_bl_end; ++i_bl) {
    ...
    for (std::int64_t i_f = 0; i_f < n_freq; ++i_f) {
      for (std::int64_t i_t = 0; i_t < n_time; ++i_t) {
        ...
        for (std::int64_t r = 0; r < n_rfi; ++r) {
          ...
          for (std::int64_t fi = 0; fi < n_int_f; ++fi) {
            ...
            for (; ti + n_lanes <= n_int_t; ti += n_lanes) {
              ...
              const auto value = Mul(MulConj(LoadU(d, amp_1 + ti),
                                             LoadU(d, amp_2 + ti)),
                                     e);
            ...
            for (; ti < n_int_t; ++ti) {
              ...
              scalar_sum += amp_1[ti] * std::conj(amp_2[ti]) * e;
            }
          }
        }
        ...
        vis(i_bl, i_f, i_t) = scale * scalar_sum;
      }
    }
  }
}
...
HWY_ATTR void
rfi_delay_kernel_opt_f32(float scale, std::int64_t i_bl_start,
                         std::int64_t i_bl_end,
                         Tensor1D<const int *> a1, Tensor1D<const int *> a2,
                         Tensor4D<const std::complex<float> *> amp,
                         Tensor4D<const float *> delay,
                         Tensor2D<const float *> freq,
                         Tensor3D<std::complex<float> *> vis,
                         std::int64_t n_rfi, std::int64_t n_int_f,
                         std::int64_t n_int_t) {
  rfi_delay_kernel_opt_tmpl<float>(scale, i_bl_start, i_bl_end, a1, a2, amp,
                                   delay, freq, vis, n_rfi, n_int_f, n_int_t);
}

HWY_ATTR void
rfi_delay_kernel_opt_f64(double scale, std::int64_t i_bl_start,
                         std::int64_t i_bl_end,
                         Tensor1D<const int *> a1, Tensor1D<const int *> a2,
                         Tensor4D<const std::complex<double> *> amp,
                         Tensor4D<const double *> delay,
                         Tensor2D<const double *> freq,
                         Tensor3D<std::complex<double> *> vis,
                         std::int64_t n_rfi, std::int64_t n_int_f,
                         std::int64_t n_int_t) {
  rfi_delay_kernel_opt_tmpl<double>(scale, i_bl_start, i_bl_end, a1, a2, amp,
                                    delay, freq, vis, n_rfi, n_int_f, n_int_t);
}
...
ffi::Future calc_rfi_delay_cpu_impl_tmpl(
    ...
    ffi::Buffer<REAL_DT, 2> freq, ffi::Result<ffi::BufferR3<AMP_DT>> vis) {
  ...
  Tensor2D<const T *> freq_tensor(freq.typed_data(), freq.dimensions()[0],
                                  freq.dimensions()[1]);
  Tensor3D<std::complex<T> *> vis_tensor(
      vis->typed_data(), vis->dimensions()[0], vis->dimensions()[1],
      vis->dimensions()[2]);
  ...
  return parallel_for(
      thread_pool, n_bl,
      [scale, a1_tensor, a2_tensor, amp_tensor, delay_tensor, freq_tensor,
       vis_tensor, n_rfi, n_int_f,
       n_int_t](std::int64_t i_bl_start, std::int64_t i_bl_end) mutable {
        if constexpr (std::is_same_v<T, float>) {
          RI_KERNELS_EXPORT_AND_DISPATCH_T(rfi_delay_kernel_opt_f32)
          (scale, i_bl_start, i_bl_end, a1_tensor, a2_tensor, amp_tensor,
           delay_tensor, freq_tensor, vis_tensor, n_rfi, n_int_f, n_int_t);
        } else {
          RI_KERNELS_EXPORT_AND_DISPATCH_T(rfi_delay_kernel_opt_f64)
          (scale, i_bl_start, i_bl_end, a1_tensor, a2_tensor, amp_tensor,
           delay_tensor, freq_tensor, vis_tensor, n_rfi, n_int_f, n_int_t);
        }
      });
...
ffi::Future calc_rfi_delay_cpu_f32_impl(
    ffi::ThreadPool thread_pool,
    ffi::BufferR1<ffi::S32> a1, ffi::BufferR1<ffi::S32> a1_sorter,
    ffi::BufferR1<ffi::S32> a1_start, ffi::BufferR1<ffi::S32> a2,
    ffi::BufferR1<ffi::S32> a2_sorter, ffi::BufferR1<ffi::S32> a2_start,
    ffi::Buffer<ffi::C64, 6> amp, ffi::Buffer<ffi::F32, 4> delay,
    ffi::Buffer<ffi::F32, 2> freq, ffi::Result<ffi::BufferR3<ffi::C64>> vis) {
  return calc_rfi_delay_cpu_impl_tmpl<ffi::C64, ffi::F32, float>(
      thread_pool, a1, a1_sorter, a1_start, a2, a2_sorter, a2_start, amp,
      delay, freq, vis);
}

ffi::Future calc_rfi_delay_cpu_f64_impl(
    ffi::ThreadPool thread_pool,
    ffi::BufferR1<ffi::S32> a1, ffi::BufferR1<ffi::S32> a1_sorter,
    ffi::BufferR1<ffi::S32> a1_start, ffi::BufferR1<ffi::S32> a2,
    ffi::BufferR1<ffi::S32> a2_sorter, ffi::BufferR1<ffi::S32> a2_start,
    ffi::Buffer<ffi::C128, 6> amp, ffi::Buffer<ffi::F64, 4> delay,
    ffi::Buffer<ffi::F64, 2> freq, ffi::Result<ffi::BufferR3<ffi::C128>> vis) {
  return calc_rfi_delay_cpu_impl_tmpl<ffi::C128, ffi::F64, double>(
      thread_pool, a1, a1_sorter, a1_start, a2, a2_sorter, a2_start, amp,
      delay, freq, vis);
}
...
calc_rfi_delay_cpu_f32(XLA_FFI_CallFrame *call_frame);
extern "C" RI_KERNELS_API XLA_FFI_Error *
calc_rfi_delay_cpu_f64(XLA_FFI_CallFrame *call_frame);

XLA_FFI_DEFINE_HANDLER_SYMBOL(calc_rfi_delay_cpu_f32,
                              calc_rfi_delay_cpu_f32_impl,
                              ffi::Ffi::Bind()
                                  .Ctx<ffi::ThreadPool>()
...
XLA_FFI_DEFINE_HANDLER_SYMBOL(calc_rfi_delay_cpu_f64,
                              calc_rfi_delay_cpu_f64_impl,
                              ffi::Ffi::Bind()
                                  .Ctx<ffi::ThreadPool>()
                                  .Arg<ffi::BufferR1<ffi::S32>>()
                                  .Arg<ffi::BufferR1<ffi::S32>>()
