...
// holds, through the (n_ant, n_ant) pair table the caller supplies. Each
// baseline integrates its own subset of the cell's time samples, every
// stride-th (see rfi_interp_common.hpp): the samples are staged time-major
// and a per-pair mask selects them, so the slow baselines of an array cost
// a fraction of the fast ones.
//
...
__global__ void __launch_bounds__(kBlock) rfi_interp_staged_kernel(
  ...
  for (INT_T tp = blockIdx.y; tp < n_tile_pairs; tp += gridDim.y) {
    ...
    // A tile pair none of whose antenna pairs is a baseline of the list has
    // nothing to stage or write: a baseline group of a variable-sampling call
    // covers a fraction of the pairs, and the rest must cost nothing.
    {
      __shared__ int any_baseline;
      if (tid == 0) any_baseline = 0;
      __syncthreads();
      for (INT_T idx = tid; idx < kTile * kTile; idx += kBlock) {
        const INT_T a1 = I * kTile + idx / kTile, a2 = J * kTile + idx % kTile;
        if (a1 < n_ant && a2 < n_ant && (v.pair(a1, a2) >= 0 || v.pair(a2, a1) >= 0))
          any_baseline = 1;
      }
      __syncthreads();
    ...
    // This thread's pairs: their stride and the mean's normalisation, once
    // per tile pair. A pair the list does not hold gets stride 1 and is never
    // written.
    INT_T pstride[kPairs];
    T pscale[kPairs];
    for (int k = 0; k < kPairs; ++k) {
      ...
      if (a1 < n_ant && a2 < n_ant) {
        INT_T bl = v.pair(a1, a2);
        if (bl < 0) bl = v.pair(a2, a1);
    ...
    for (INT_T cell = blockIdx.x; cell < n_cells; cell += gridDim.x) {
      ...
      for (INT_T r = 0; r < n_rfi; ++r) {
        for (INT_T s0 = 0; s0 < n_s; s0 += chunk) {
          ...
          // Which of the chunk's time samples each pair integrates: one bit
          // per time sample from the chunk's first, v_lo.
          const INT_T v_lo = s0 / n_int_f;
          unsigned mask[kPairs];
          for (int k = 0; k < kPairs; ++k) mask[k] = stride_mask(v_lo, n_int_t, pstride[k]);
          // Multiply: pair k of this thread is (i = tid / kTile + 8 k, j = tid % kTile).
          INT_T u = s0 % n_int_f, vv = v_lo;
          for (INT_T s = 0; s < cs; ++s) {
            const unsigned bit = 1u << (vv - v_lo);
            if (++u == n_int_f) { u = 0; ++vv; }  // the next sample's (u, v)
            const Cplx<T> b = TJ[s * kTile + j_local];
            Cplx<T> db{0, 0};
            if constexpr (JVP) db = DJ[s * kTile + j_local];
            for (int k = 0; k < kPairs; ++k) {
              if (!(mask[k] & bit)) continue;
              const int i_local = tid / kTile + k * (kBlock / kTile);
      ...
      for (int k = 0; k < kPairs; ++k) {
        ...
        if (bl >= 0) out(bl, f, t) = cscale(pscale[k], acc[k]);
        if (a1 != a2) {
          const INT_T bl2 = v.pair(a2, a1);
          if (bl2 >= 0) out(bl2, f, t) = cscale(pscale[k], cconj(acc[k]));
        }
      }
    }
  }
...
ffi::Error calc_rfi_interp_gpu_dispatch(
  ...
  auto views = make_views<T, INT_T>(a1, a2, pair, stride, amp, phase, delay, w_freq,
                                    start_freq, w_time, start_time, dnu, dt, freqs);
  const auto a = amp.dimensions();
  ...
  rfi_interp_staged_kernel<T, JVP, INT_T><<<grid, kBlock, shared, stream>>>(
      views, amp_dot_view, out_view, chunk, n_tiles, n_tile_pairs);
...
ffi::Error calc_rfi_interp_gpu_impl_tmpl(
  ...
  if (amp.element_count() < limit && out->element_count() < limit)
    return calc_rfi_interp_gpu_dispatch<JVP, T, std::int32_t>(
        stream, a1, a2, pair, stride, amp, amp_dot, phase, delay, w_freq, start_freq,
        w_time, start_time, dnu, dt, freqs, out);
...
#define RI_INTERP_GPU_ENTRY(NAME, JVP, AMP_DT, REAL_DT, T, AMP_T, R4, R3, R1, DOT_PARAM, DOT_ARG) \
  ffi::Error NAME##_impl(                                                       \
      ...
      interp_index_t stride, AMP_T amp DOT_PARAM,                                \
    ...
    return calc_rfi_interp_gpu_impl_tmpl<JVP, AMP_DT, REAL_DT, T>(               \
        ...
        amp, DOT_ARG, phase, delay, w_freq, start_freq, w_time, start_time, dnu,  \
        dt, freqs, out);                                                         \
  }
